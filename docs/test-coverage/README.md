...
ワークストリームは**番号順に直列**で進めます。各ワークストリームは 1 つ前のワークストリームのブランチをベースに作業します。
...
**W0 は必ず最初に完了させてください。** W1〜W6 が W0 で追加するテストヘルパに依存します。
W1〜W6 の担当ファイルは互いに素になるよう分割してあるため、直列に積んでもコンフリクトは起きない想定です。
...
ブランチを積み上げる都合上、前のワークストリームが追加したファイルは手元に見えますが、**それらを変更しないでください。**各 PR の差分をそのワークストリームの担当分だけに保つためです。
...
**ワークストリームごとに個別の PR を出してレビューします。** ブランチは前のワークストリームのものをベースに積み上げる、いわゆるスタック PR の形になります。

> **作業ブランチの作成は Claude Code Web のセッションが自動で行います。**
> 各ワークストリームのエージェントが自分で `git checkout -b` する必要はありません。
> 以下はセッションを起動する際にどのブランチをベースに指定すればよいかの対応表です。

| WS | ベースにするブランチ（= PR の base） |
|---|---|
| W0 | `claude/test-coverage-improvement-plan-x14ldp`（統合ブランチ） |
| W1 | W0 の作業ブランチ |
| W2 | W1 の作業ブランチ |
| W3 | W2 の作業ブランチ |
| W4 | W3 の作業ブランチ |
...
各ワークストリームは、自分の作業ブランチから**ベースブランチを base とした PR** を 1 本出します。
こうすると各 PR の差分がそのワークストリームの変更だけになり、レビュー単位が小さく保たれます。

### 進め方

1. W0 から順に着手します。あるワークストリームの作業ブランチが push されたら、次のワークストリームのセッションをそのブランチをベースに起動できます。**前の PR のマージを待つ必要はありません。**
2. PR のマージは**前から順に**行います。W0 → W1 → … の順にマージすると、後続 PR の base が自動的に繰り上がります。
3. レビュー指摘で前のワークストリームのブランチが更新されたら、後続のブランチはそれを取り込んでからテストを再実行してください。

   ```bash
   git fetch origin && git merge origin/<前のワークストリームのブランチ>
   ```

4. W6 まですべてマージされたら、統合ブランチから `main` 向けの PR を 1 本出します。
...
- **ファイルの作成・編集には Edit / Write ツールを使い、シェルの heredoc（`cat > file <<EOF`）は可能な限り使わないでください。**
- heredoc は Edit / Write で正しく書き込めない場合のみのフォールバックとします。
...
すべて通ったら作業ブランチを push し、ベースブランチ（1 つ前のワークストリームのブランチ。W0 は統合ブランチ）を base とした PR を 1 本作成します（draft にしないこと）。
PR 作成後は CI（`Test & Lint`）が緑になるまでが 1 ワークストリームの完了です。**bot のレビューコメントには、ユーザーからの指示があるまで対応しません。**

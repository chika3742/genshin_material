  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Stack(
      children: [
        ...
        if (state.groups.isEmpty) Padding(
...
class _ArtifactBookmarkList extends ConsumerWidget {
  const _ArtifactBookmarkList();

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final state = ref.watch(artifactBookmarkListViewModelProvider);

    return ReorderableListView.builder(
      itemCount: state.bookmarks.length,
      itemBuilder: (context, index) {
        final group = state.bookmarks[index];

        return _GroupContainer(
          key: ValueKey(group.hash),
          index: index,
          group: group,
          child: switch (group.bookmarks.first) {
            final BookmarkWithArtifactSetDetails b => _ArtifactSetDetails(b),
            final BookmarkWithArtifactPieceDetails b => _ArtifactPieceDetails(b),
            _ => throw Exception("Unreachable"),
          },
        );
      },
      onReorder: ref.read(artifactBookmarkListViewModelProvider.notifier).reorder,
    );
  }
}

...
void main() {
  ...
  test("Check for updates", () async {
    ...
    when(client.get(Uri.parse(assetReleasesUrl))).thenAnswer((_) async {
      return http.Response('{"channels": {"staging": {"createdAt": "2024-01-01T00:00:00Z", "dataVersion": "test", "channel": "staging", "distUrl": ""}}}', 200);
    });
    ...
    // current version is the same as latest
    await File("${Directory.current.path}/version.json")
        .writeAsString('{"createdAt": "2024-01-01T00:00:00Z", "dataVersion": "test", "channel": "staging", "distUrl": ""}');
    await updater.checkForUpdate();
    expect(updater.isUpdateAvailable, false);

    // latest version is newer than current
    when(client.get(Uri.parse(assetReleasesUrl))).thenAnswer((_) async {
      return http.Response('{"channels": {"staging": {"createdAt": "2024-06-01T00:00:00Z", "dataVersion": "test", "channel": "staging", "distUrl": ""}}}', 200);
    });
    await updater.checkForUpdate();
    expect(updater.isUpdateAvailable, true);
  });

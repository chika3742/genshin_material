...
- Classes that have no access to a `Ref` take the **value**, not a Remote Config object: the caller reads it from `remoteConfigProvider` and passes it in. `AssetUpdater.checkForUpdate` takes `minimumSchemaVersion` as a method argument this way.
- A **kill switch is a precondition, not a value**: it says whether the work may happen at all, so it is evaluated in the provider that hands out the service, and is never carried into the class as a constructor argument. Mind the asymmetry when catching the result: reading a failed *synchronous* provider wraps the cause in `ProviderException` (from `package:flutter_riverpod/misc.dart`), while `.future` on an asynchronous one rethrows the cause untouched. `getErrorMessage` (`lib/ui_core/error_messages.dart`) unwraps the former, so UI code never has to.

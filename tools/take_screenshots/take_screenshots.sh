...
function takeScreenshotsAndroid() {
    ...
    adb -s $emu_serial emu kill || true

    ~/Library/Android/sdk/emulator/emulator @"$1" -port $port -netdelay none -netspeed full > /dev/null &
    EMU_PID=$!
    echo "Android emulator started. Waiting for ADB can see the device..."
    adb -s $emu_serial wait-for-device
    # shellcheck disable=SC2016
    adb -s "$emu_serial" shell 'while [[ -z $(getprop sys.boot_completed) ]]; do sleep 1; done'

    # suppress "isn't responding" / "has stopped" dialogs, which corrupt screenshots
    adb -s $emu_serial shell settings put global hide_error_dialogs 1
    for locale in "${locales[@]}"; do
        takeScreenshots "$screenshot_dir" "$locale" "{index}_${locale}.png" $emu_serial android
    done
    echo "Stopping the Emulator server..."
...
function takeScreenshotsApple() {
    ...
    # wait for boot
    echo "Waiting for the Simulator startup..."
...
function takeScreenshots() {
    ...
    echo "Running Flutter screenshot test driver..."
    fvm flutter drive --driver\
      "$DIR/driver.dart"\
      -d "$device_id"\
      --target "$DIR/take_screenshots_test.dart"\
      --dart-define SCREENSHOT_MODE=true\
      --dart-define LOCALE="$locale" > /dev/null
